    ...
    async fn lock_contains_prompts_and_materializes_features() {
        ...
        Arc::make_mut(&mut sc.original_config_do_not_use).compact_prompt = Some("resolved compact prompt".to_string());
        ...
        assert_eq!(lock.compact_prompt, sc.original_config_do_not_use.compact_prompt);
        assert_eq!(lock.model, Some(sc.step_settings.collaboration_mode.model().to_string()));
        assert_eq!(
        ...
        assert_eq!(
            ...
            turn_metadata_includes_tool_info: Some(sc.original_config_do_not_use.tool_registry.turn_metadata_includes_tool_info),
            })
        );
        ...
        assert_eq!(lockfile.version, crate::spine::config_snapshot::CONFIG_LOCK_VERSION);
    }
    ...
    async fn lock_skips_session_values_when_model_catalog_fields_are_not_saved() {
        ...
        Arc::make_mut(&mut sc.original_config_do_not_use).compact_prompt = Some("catalog compact prompt".to_string());
...
"#,
    ...
    #[tokio::test]
    async fn snapshot_survives_source_mutation_deletion_and_appearance()
    -> std::io::Result<()> {
