...
async fn cold_resume_v033_sampling_archive_preserves_branch_and_continues() -> Result<()> {
    ...
    let response = responses::mount_sse_once(&server, responses::sse(vec![
    ...
    let fixture = std::fs::read_to_string(codex_utils_cargo_bin::find_resource!("tests/fixtures/spine/rollout-v033.jsonl")?)?;
    ...
    let mut records = fixture.lines().map(serde_json::from_str::<serde_json::Value>)
        .collect::<serde_json::Result<Vec<_>>>()?;
    for record in &mut records {
        if matches!(record["type"].as_str(), Some("session_meta" | "turn_context")) {
    ...
    let archive = records.iter().map(serde_json::to_string).collect::<serde_json::Result<Vec<_>>>()?.join("\n");
    std::fs::write(&rollout, format!("{archive}\n"))?;
    let test = spine_test_codex().with_model("gpt-5.4")
        .resume(&server, home, rollout).await?;
    test.submit_turn("Continue the preserved legacy migration branch.").await?;
    let request = response.single_request().body_json().to_string();
    assert!(request.contains("Preserve this historical user request"), "{request}");
    assert!(request.contains("legacy migration branch"), "{request}");
    assert!(request.contains("Continue the preserved legacy migration branch."), "{request}");
