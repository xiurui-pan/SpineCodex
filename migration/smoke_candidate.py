...
class MockModel(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if encoding == "zstd":
            body = subprocess.run(
                ["zstd", "-dc"], input=body, capture_output=True, check=True
            ).stdout
        elif encoding == "gzip":
        ...
        events = [
            {"type": "response.created", "response": {"id": response_id}},
            {
                ...
                "item": {
                    ...
                    "role": "assistant",
                    "content": [
                        {"type": "output_text", "text": "Candidate smoke completed."}
                    ],
                },
            },
    ...
    def turn(self, thread_id, text):
        ...
        while True:
            message = (
                ...
                if self.pending
                else self.messages.get(timeout=30)
            )
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="spine-candidate-") as temporary:
        ...
        (
            home / "config.toml"
        ).write_text(f'''spine_config_file = "{sdk_source.as_posix()}"
model = "gpt-5.4"
...
''')
        environment = dict(
            os.environ, CODEX_HOME=str(home), MIGRATION_MOCK_KEY="synthetic-test-key"
        )
        environment["NO_PROXY"] = "127.0.0.1,localhost"
        environment["no_proxy"] = environment["NO_PROXY"]
        probe = subprocess.run(
            [str(binary), "mcp-server"],
            input=json.dumps(
                {
                    "jsonrpc": "2.0",
                    "id": 1,
        ...
        product = next(
            json.loads(line)["result"]["serverInfo"]["version"]
        ...
        with (args.log_dir / "app-server.stderr").open("w") as stderr:
            ...
            started = app.request(
                "thread/start", {"cwd": str(workspace), "approvalPolicy": "never"}
            )
        ...
        outputs = [
            item
            for item in MockModel.requests[1]["input"]
            if item.get("call_id") == "smoke-exec-call"
            and item["type"] == "custom_tool_call_output"
        ]
        assert len(outputs) == 1, outputs
        execution = json.loads(outputs[0]["output"][-1]["text"])
        assert (execution["exit_code"], execution["output"]) == (
            0,
            "spine-package-smoke",
        ), execution
        assert "spine" in json.dumps(MockModel.requests[0]["tools"])
        assert "first isolated migration smoke turn" in json.dumps(
            MockModel.requests[2]["input"]
        )
        assert "second isolated migration smoke turn" in json.dumps(
            MockModel.requests[2]["input"]
        )
        (args.log_dir / "requests.json").write_text(
            json.dumps(MockModel.requests, indent=2)
        )
    model.shutdown()
    print(
        json.dumps(
            {
                ...
                "requests": 4,
                "fork": "passed",
                "savedConfigAfterSourceRemoval": "passed",
                "codeModeExecution": "passed",
                "resume": "passed",
                "shutdown": "passed",
            }
        )
